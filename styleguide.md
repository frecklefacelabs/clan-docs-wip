...
Depending on context; if this is inside a list of steps:

> 3. Create a new webserver machine in your Clan:

~~~nix
clan machines create --name webserver
~~~

Or if it's a standalone explanation, start with the word To:

> To create a new webserver machine in your Clan:
...
Always paste **tested** code in from the editor or shell. Do not type code directly into the document; even the best developers make mistakes. Then, if necessary, replace hardcoded constants and such with placeholders; for example, you might paste in:

~~~nix
clan.networking.targetHost = "192.168.1.10"
~~~

And then replace the IP address with a placeholder:

~~~nix
clan.networking.targetHost = "<IPADDRESS>"
~~~
...
- Test every example on a clean environment (this catches hidden dependencies on pre-existing configurations)
...
It's okay to call out placeholders; otherwise the other comments are not necessary.

~~~nix
{
  clan.networking.targetHost = "<IPADDRESS>"; # Replace with your machine's IP
...
- Use heading levels (`#`, `##`, `###`, etc.) to reflect document hierarchy, not to control text size.
...
Inline code is marked using single backticks: \`this\`, which renders as `this`.

---

#### Code Snippets

Code snippets use triple backticks and, when applicable, specify a language on the first line, which renders with syntax highlighting:
